<template>
  <v-card width="590" class="elevation-5" height="350">
    <v-row>
      <v-col md="6">
        <div class="d-flex align-center">
          <div class="ml-3">
            <img
              class="emoji-img"
              src="https://cdn.shopify.com/s/files/1/1061/1924/products/Briefcase_Emoji_1024x1024.png?v=1571606066"
            />
          </div>
          <div class="pa-5">
            <div>Balance Business</div>
            <div>
              <!-- <span class="actual-number-balance">{{
                setSickAndAnnualBalances[0].businessLeaveBalance
              }}</span> -->
    ...
    <v-row>
      <table class="table-requests">
        ...
        <tbody v-for="(item, index) in filteredData" :key="index">
          <template v-if="item.leaveType == 'business'">
            <tr>
    ...
    <BusinessLeaveForm
      ...
      :phoneForEmployee="phoneForEmployee"
    />
    <UpdateLeaveEmployee
...
import BusinessLeaveForm from "../dialogs/BusinessLeaveForm.vue";
...
export default {
  ...
  methods: {
    ...mapActions(["actionForGettingAllLeaveWork", "setSickAndAnnualBalances"]),
    ...
    viewHistory() {
      this.$router.push(
        "/hrpages/businessHistory/" + this.employeeUuid + "--" + "business"
      );
    },
  },
  computed: {
  ...
  components: { BusinessLeaveForm, UpdateLeaveEmployee },
};
</script>
<style scoped>
.emoji-img {
