          ...
          <div class="pa-5">
            ...
            <div>
              <!-- <span class="actual-number-balance">{{
                setSickAndAnnualBalances[0].sickBalanceLeave
              }}</span> -->
              <span class="left-days-balance" v-if="setSickAndAnnualBalances[0]"
                >{{ setSickAndAnnualBalances[0].sickLeaveBalance }}/14</span
              >
      ...
      <v-col md="6">
        <div class="emoji-box">
          <v-btn
            ...
            @click="sickLeaveDialog = !sickLeaveDialog"
            v-if="$store.getters.tokenValue.profileType != 'MANAGER'"
            >Request Sick Leave</v-btn
          >
        </div>
      </v-col>
    </v-row>
    ...
    <v-row>
      <table class="table-requests">
        ...
        <tbody v-for="(item, index) in filteredData" :key="index">
          <template v-if="item.leaveType == 'sick'">
            <tr>
        ...
        <!-- <UpdateLeaveEmployee :updateLeaveDialog.sync="updateLeaveDialog" /> -->
        <!-- <UpdateLeaveEmployee
          :updateLeaveDialog.sync="updateLeaveDialog"
          :employeeUuid="$route.params.hrHom"
        /> -->
      </table>
      <UpdateLeaveEmployee
        :updateLeaveDialog.sync="updateLeaveDialog"
...
export default {
  ...
  methods: {
    ...
    viewHistory() {
      this.$router.push(
        "/hrpages/businessHistory/" + this.employeeUuid + "--" + "sick"
      );
    },
  ...
  components: { SickLeaveForm, UpdateLeaveEmployee },
};
</script>
<style scoped>
.showLeaveBtn {
